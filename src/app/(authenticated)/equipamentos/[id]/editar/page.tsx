...
const schema = z.object({
  ...
  situacao: z.enum(["ativo", "fora_de_uso", "em_manutencao", "descartado"]),
});
...
export default function EditarEquipamentoPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    async function load() {
      const [eqRes, fabRes, modRes, tipRes, clsRes, uniRes, labRes] = await Promise.all([
      ...
      if (eqRes.ok) {
        ...
        form.reset({
          ...
          classificacaoId: eq.classificacaoId,
          unidadeId: eq.unidadeId,
          laboratorioId: eq.laboratorioId,
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        toast.error(err.error || "Erro ao atualizar equipamento.");
        return;
      }
      ...
      toast.error("Erro de conexão ao atualizar equipamento.");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <Button variant="ghost" size="icon-sm" onClick={() => router.push(`/equipamentos/${id}`)}>
